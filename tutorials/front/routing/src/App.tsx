...
const UserPage = () => {
  ...
  const userId = match?.params.userId;   // userID est un string 
  if (!userId) return <p>User not found</p>;

  const user = users.find((user) => user.id.toString() === userId);   // user.id est un number.toString() pour convert le type
